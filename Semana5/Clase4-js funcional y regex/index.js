...
const emailsGuille = ["john.doe@example.com",
...
function clearEmail(email) {
   if ((email.match(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/))) {
    // return console.log(`"match" ${email}`);
    return email;
  } else if (email.match(/^[_a-zA-Z0-9-]+(.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(.[a-zA-Z0-9-]+)*(.[a-zA-Z]{2,3})$/)){
    ...
    const domain = email.match(/@[\w-]+\.+[a-zA-Z]{2,7}(?:.[a-zA-Z]{2,7})|@[\w-]+\.+[a-zA-Z]{2,7}/)
